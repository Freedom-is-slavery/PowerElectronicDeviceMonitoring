...
err_t tcp_client_connected(void *arg, struct tcp_pcb *tpcb, err_t err)
{
	...
	if(err==ERR_OK)   
	{
		...
		if(es) //内存申请成功
		{
			...
			TestStatus = STATUS_IS_TCP_CLIENT;	//标记当前运行的TCP_Client,并且连接已就绪
			err=ERR_OK;
		}
		else
		{ 
...
void TCP_Client_send_ADCValue(vu16 *value)
{
	...
	if(es!=NULL)  //连接处于空闲可以发送数据
	{
		if(tcp_client_flag&(1<<7))	//判断是否有数据要发送 
		{
			...
			es->p=pbuf_alloc(PBUF_TRANSPORT, TCP_CLIENT_TX_BUFSIZE, PBUF_POOL);	//申请内存 
			pbuf_take(es->p,(char*)tcp_client_sendbuf,strlen((char*)tcp_client_sendbuf));	//将tcp_client_sentbuf[]中的数据拷贝到es->p_tx中
			tcp_client_senddata(tcppcb,es);//将tcp_client_sentbuf[]里面复制给pbuf的数据发送出去
			tcp_client_flag&=~(1<<7);	//清除数据发送标志
			if(es->p)pbuf_free(es->p);	//释放内存
		}
		else if(es->state==ES_TCPCLIENT_CLOSING)
		{ 
 			tcp_client_connection_close(tcppcb,es);//关闭TCP连接
		}
	}
	else
	{ 
		tcp_abort(tcppcb);	//终止连接,删除pcb控制块
	}
}
